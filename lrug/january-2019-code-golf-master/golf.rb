module Golf
  ...
  def hole2(l)
    (1..l).to_a.map{|n|n%15==0?'fizzbuzz':n%3==0?'fizz':n%5==0?'buzz':n}
  end # Hole 2
  ...
  def hole3(s, x)
    ...
    h=(1..26).zip(('A'..'Z')).to_h
    s.chars.map { |l| h.key(l.upcase) } .map { |k| h[k+x] } .join('')

    #keep casing and spaces
  ...
  def hole5(l)
    (0..l).select{|n|n.to_s(2).chars.count('1')%2==0}
  end # Hole 5
